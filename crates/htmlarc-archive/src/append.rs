...
impl ArchiveAppender {
    ...
    /// On Windows, all memory mappings of the file must be dropped first, including
    /// those held by document handles or other processes: Windows rejects truncation
    /// of a mapped file. Concurrent appenders are not supported on any platform.
